...
describe "HTTP server" do
  ...
  before(:all) do
    ...
    @profile = @account.add_profile(name: 'profilemy', queue: 'queuetest')
  end
  ...
  it "should return 404 if there is no profile with such queue for the account" do
    get '/', {}, {'HTTP_PERSONALPORT' => 234567, 'HTTP_PERSONALQUEUE' => '111'}
    expect(last_response.status).to eql(404)
    expect(last_response.body).to eq('No profile')
  end
